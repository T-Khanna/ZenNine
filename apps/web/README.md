...
- Board edits are synced to backend session event append API (initial contract).
- Input modes:
...
- Backend integration currently covers ping plus session event append/list sync. Validation endpoint and replay hydration flows are not wired into the UI yet.
